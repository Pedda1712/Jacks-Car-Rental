"""
Copyright (C) 2025 Peter Preinesberger.
...
_p3.c_prime_given_c.argtypes = (ctypes.c_int,
                                ctypes.c_int,
                                ctypes.c_int,
                                ctypes.c_int,
                                ctypes.c_int)
...
def _acc_c_prime_given_c(c_prime, c, mean_requests, mean_returns, MAX):
    global _p3
    result = _p3.c_prime_given_c(ctypes.c_int(c_prime),
                                 ctypes.c_int(c),
                                 ctypes.c_int(mean_requests),
                                 ctypes.c_int(mean_returns),
                                 ctypes.c_int(MAX))
    return result
...
def partial_p(
        ...
        return_mean: int,
        new_state: int,
        old_state_after_action: int
):
    """
    Partial function for the 3-argument p.

    Computes the state transition probability at just one of the locations.
    parameters:
    ...
    """
    return _acc_c_prime_given_c(new_state,
                                old_state_after_action,
                                request_mean,
                                return_mean,
                                MAX_CARS)
...
def p(new_state: (int, int), old_state: (int, int), a: int):
    ...
    """
    partial_1 = partial_p(RENTAL_REQUEST_MEANS[0],
                          RENTAL_RETURN_MEANS[0],
                          new_state[0],
                          old_state[0] - a)
    partial_2 = partial_p(RENTAL_REQUEST_MEANS[1],
                          RENTAL_RETURN_MEANS[1],
                          new_state[1],
                          old_state[1] + a)
...
def prob_rental(request_mean: int, max_cars: int, rent_num: int):
    """
    Calculate the probability of renting out a specific number of cars.
...
def r(old_state: (int, int), a: int):
    """
    Calculate expected reward/cost function of the problem.
    ...
    for i in range(0, max_cars[0]+1):
        rewards += (prob_rental(RENTAL_REQUEST_MEANS[0], max_cars[0], i)
                    * i * PROFIT_FROM_RENTING)
    for i in range(0, max_cars[1]+1):
        rewards += (prob_rental(RENTAL_REQUEST_MEANS[1], max_cars[1], i)
                    * i * PROFIT_FROM_RENTING)
    return rewards
...
def q(
    ...
    """Calculate state-action value based on one-step search."""
    v = r(state, action)
    for s_prime in S:
...
def A(state: (int, int)):
    """Calculate permissable actions in state."""
    s = []
...
# Policy Iteration Algorithm (from the book directly)
...
for on_iteration in range(ITERATION_STEPS):
    ...
    while True:
        ...
        for (idx, s) in enumerate(S):
            ...
            for s_prime in S:
                new_v_temp += (p(s_prime, s, policy[s])
                               * value[s_prime]
                               * DISCOUNT)
            value[s] = new_v_temp
